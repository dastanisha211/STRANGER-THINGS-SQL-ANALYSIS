...
Select c.name as character_name,e.event_name as event_name from characters c join relationships r 
on c.character_id=r.character1_id or c.character_id=r.character2_id
join events e
on e.event_id=r.relationship_id;
...
Select c.name as character_name,e.season as season from characters c join relationships r 
on c.character_id=r.character1_id or c.character_id=r.character2_id
join events e
on e.event_id=r.relationship_id
where e.season=1 or e.season=2;
...
Select round(avg(age),0) as average_age from Characters where hometown='Hawkins';

#10)Rank characters by age in descending order?
Select character_id, name, rank() over(order by age desc) as rnk
from Characters;
